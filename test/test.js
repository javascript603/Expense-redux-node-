...
describe('Load a Page', function () {
    ...
    it('EditExpense', done => {
        nightMareInstance = nightmare.goto('http://localhost:3000/products');
        nightMareInstance
            .evaluate(() => {
    ...
    it('Add Label', done => {
        nightMareInstance = nightmare.goto('http://localhost:3000/products');
        nightMareInstance
            .click('.react-bs-container-body table tbody tr:nth-child(1) td:nth-child(7) .fa-edit')
            .wait(400)
            .evaluate(() => {
                return document.querySelectorAll('#EditForm').length;
            })
            .then((count)=>{
                  if (count != 1) {
                    assert.equal(301, 311);
                    done();
                } else {
                    nightMareInstance
                        .evaluate(() => {
                            return document.querySelectorAll('#EditForm .col-md-8 input').length;
                        })
                        .then((firstcount) => {
                            nightMareInstance
                                .click('#EditForm .col-md-3 button')
                                .wait(300)
                                .evaluate(() => {
                                    return document.querySelectorAll('#EditForm .col-md-8 input').length;
                                })
                                .then((secondcount) => {
                                    if (secondcount !== firstcount + 1) {
                                        assert.equal(401, 402);
                                        done();
                                    } else { 
                                        let label = `label${firstcount}`;
                                        nightMareInstance
                                            .type(`input[name=${label}]`, "hello")
                                            .wait(200)
                                            .evaluate(() => {
                                                return document.querySelector("form>.row>.row:last-child input").value
                                            })
                                            .then((text) => {
                                                nightMareInstance
                                                    .click("button[type='submit']")
            ...
            .then((formcount) => {
                if (formcount == 0) {
                    ...
                    nightMareInstance
                        ...
                        .wait(500)
                        .evaluate(() => {
                            return document.querySelector('.react-bs-container-body table tbody tr:nth-child(1) td:nth-child(7) div').innerText;
                        })
                        .then(function (txt) {
                            if(txt=="Approve"){
                                assert.equal(1, 1);
    ...
    it('Sort-Filter', done => {
        let searchField = [{ price1: "1", price2: "400" }];
